...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <section className="shell home-section analytics-tease"><div><p className="kicker">Catalog coverage</p><h2>Which payment networks appear across the card index?</h2><p>Current catalog categories make issuer terminology easier to compare. Counts measure programs, not market share.</p><Link className="button secondary" href="/analytics">Inspect the distribution</Link></div><div className="mini-bars" aria-label="Payment network distribution preview">{distribution(snapshot.cards, "network").slice(0, 6).map((item) => <div key={item.label}><span>{item.label}</span><i style={{ width: `${(item.value / snapshot.cards.length) * 100}%` }} /><strong>{item.value}</strong></div>)}</div></section>
    </>
  );
}
