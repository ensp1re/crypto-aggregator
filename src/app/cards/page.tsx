...
export const metadata: Metadata = { title: "Explore crypto cards", description: "Research 42 crypto-card programs with explicit source and verification status." };
  ...
  return (
    <div className="shell page-stack catalog-page">
      <header className="editorial-header">
        <div><p className="kicker">Card index / observation 17.07.26</p><h1>Forty-two programs. One honest starting point.</h1></div>
        <p>Search the broad discovery index, then inspect what is known, what is merely reported, and which official source must confirm it.</p>
      </header>
      ...
      <div className="result-line"><p><strong>{results.length}</strong> of {snapshot.cards.length} programs</p><p>Sorted alphabetically. Commercial status has no effect.</p></div>
      <section className="catalog-list" aria-label="Crypto card programs">
        {results.length ? results.map((card) => <CardRow card={card} key={card.id} />) : <div className="empty-state"><h2>No matching observation</h2><p>Clear filters or try an issuer, region, or asset name.</p></div>}
      </section>
    </div>
  );
}
